...
class ProcessamentoPrata:
    def __init__(self, robo: RoboComunicador):
        ...
        self.sal_lgpd = os.environ.get("LGPD_SALT", "safedriver_2026_token")
    ...
    def carregar_base_geografica(self):
        ...
        try:
            ...
            raise Exception("Erro ao carregar malha geografica do R2")
    ...
    def executar(self, ano):
        ...
        try:
            self.robo.enviar_relatorio_operacional(f"Iniciando Prata {ano}")
            ...
            mascara_gps = (df['LATITUDE'].notnull()) & (df['LATITUDE'] != 0) & (df['LONGITUDE'] != 0)
            df.loc[mascara_gps, 'H3_INDEX'] = [
                h3.latlng_to_cell(lat, lon, 8) 
                for lat, lon in zip(df.loc[mascara_gps, 'LATITUDE'], df.loc[mascara_gps, 'LONGITUDE'])
            ]

            df_geo = self.carregar_base_geografica()

            mascara_recuperacao = df['H3_INDEX'].isnull()
            if mascara_recuperacao.any():
                df_recuperado = pd.merge(
                    df.loc[mascara_recuperacao].drop(columns=['H3_INDEX']),
                    df_geo[['CHAVE_LOCALIDADE', 'H3_INDEX']],
                    on='CHAVE_LOCALIDADE',
                    how='inner'
                )
                df = pd.concat([df.loc[~mascara_recuperacao], df_recuperado], ignore_index=True)
            ...
            df = pd.merge(df, df_geo[colunas_enriquecimento], on='H3_INDEX', how='inner')

            pesos = {
                'HOMICIDIO DOLOSO': 10, 'LATROCINIO': 10, 'ROUBO': 5, 
                'ESTUPRO': 8, 'FURTO': 2, 'TRAFICO DE ENTORPECENTES': 4
            }
            df['PESO_CRIME'] = df['NATUREZA'].apply(lambda x: pesos.get(self.normalizar(x), 1))
            ...
            df_final.to_parquet(buffer_prata, index=False)
            self.s3.put_object(Bucket=self.bucket, Key=caminho_prata, Body=buffer_prata.getvalue())

            self.robo.enviar_relatorio_operacional(f"Sucesso Prata {ano}", {"Registros": len(df_final)})
            return True

        except Exception:
            self.robo.enviar_alerta_tecnico(f"Erro na Prata {ano}", traceback.format_exc())
            return False

def iniciar_prata(robo, ano):
