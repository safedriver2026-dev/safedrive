    ...
    def processar(self):
        ...
        prata = (
            ...
            .with_columns([
                ...
                ).cast(pl.Int8).alias("SEMANA_PAGAMENTO"),
                pl.when(
                    pl.col("N").str.to_uppercase().str.contains("VEICULO|AUTO|CARGA")
                ).then(pl.lit("MOTORISTA"))
                 .when(
                    pl.col("N").str.to_uppercase().str.contains("TRANSEUNTE|PEDESTRE")
                 ).then(pl.lit("PEDESTRE"))
                 .when(
                    pl.col("N").str.to_uppercase().str.contains("MOTO")
                 ).then(pl.lit("MOTOCICLISTA"))
                 .when(
                    pl.col("N").str.to_uppercase().str.contains("BICICLETA")
        ...
        # Camada de validação: crimes reais (prata) vs escore médio (ouro), ambos por H3
