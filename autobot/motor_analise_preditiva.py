...
class MotorAnalitico:
    ...
    def processar_pipeline(self, df_raw):
        df = df_raw.copy()
        ...
        fato = df.groupby(['H3_INDEX', 'TURNO', 'DATA_DT']).agg({'SCORE_PONDERADO': 'sum', 'LAT': 'mean', 'LON': 'mean'}).reset_index()
        ...
        fato.to_parquet(self.pastas["ouro"] / "inteligencia_consolidada.parquet", index=False)
        fato[['H3_INDEX', 'LAT', 'LON', 'TURNO', 'IS_PAGAMENTO', 'PREDICAO_RISCO']].to_csv(self.pastas["ouro"] / "dashboard_risco_consolidado.csv", index=False)
