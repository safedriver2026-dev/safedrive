...
class ArquitetoSafeDriverPrata:
    ...
    def _limpar_colunas_texto(self, df):
        """Limpa acentos, força caixa alta e remove lixo de todas as colunas string."""
        cols = [c for c, t in zip(df.columns, df.dtypes) if t == pl.Utf8]
    ...
    def processar(self):
        ...
        try:
            # 1. Censo (Leitura com separador e encoding correto)
            ...
            ]).pipe(self._limpar_colunas_texto)

            # 2. Ruas (Leitura direta por arquivo extraído)
            ...
            with zipfile.ZipFile(zip_f, 'r') as z:
                ...
                for f in json_files:
                    ...
                    # Lê o arquivo específico, não o diretório todo
                    ...
                    # Limpa logo após ler
                    if "/" in f: shutil.rmtree(os.path.join(self.temp_extract_dir, f.split('/')[0]), ignore_errors=True)
                    else: os.remove(path_json)

            df_ruas = pl.concat(vias_list).with_columns(self._limpar_cd_setor("CD_SETOR")).pipe(self._limpar_colunas_texto)

            # 3. O Pulo do Gato: Spatial Join apenas por H3 Único
            ...
            """).pl().pipe(self._limpar_colunas_texto)

            # 4. Salvando Malha Social e Infraestrutura
            df_vias_final = df_ruas_h3.drop("CID_CENSO").join(df_geo_ref, on="H3_INDEX", how="left").pipe(self._limpar_colunas_texto)
            ...
            # Export Infra (CNAE)
            infra_pqs = glob.glob(f"**/CNPJ_SP_HISTORICO_LOTE_*.parquet", recursive=True)
            if infra_pqs:
                df_infra = pl.scan_parquet(infra_pqs).filter(pl.col("lat").is_not_null()) \
                    .with_columns(pl.col("cnae_fiscal_principal").cast(pl.Utf8).str.slice(0, 2).alias("CNAE_DIV")).collect() \
                ...
                df_infra = df_infra.rename({c: f"INFRA_DIV_{c}" for c in df_infra.columns if c != "H3_INDEX"}) \
                    .join(df_geo_ref, on="H3_INDEX", how="left").pipe(self._limpar_colunas_texto)
                df_infra.write_parquet(f"{self.prata_dir}/PRATA_MALHA_INFRA_AGREGADA.parquet")

            # Upload Final
            for f in glob.glob(f"{self.prata_dir}/*.parquet"):
                self.s3.upload_file(f, self.bucket, f"datalake/prata/malha_trusted/{os.path.basename(f)}")
            print(f"✅ Sucesso em {time.time() - tempo_inicio:.2f}s")
