...
attributes=name,highway,waterway,aerialway,barrier,man_made,z_order
    ...
    def processar(self):
        ...
        try:
            ...
            query = f"""
                SELECT 
                    CAST(osm.osm_id AS VARCHAR) as CD_FACE,
                    osm.name as RUA, 
                    CAST(NULL AS VARCHAR) as NM_MUN,
                    CAST(NULL AS VARCHAR) as NM_BAIRRO,
                    CAST(ST_Y(ST_Centroid(osm.geom)) AS FLOAT) as LAT, 
                    CAST(ST_X(ST_Centroid(osm.geom)) AS FLOAT) as LON
                FROM ST_Read('{osm_pbf}', layer='lines', open_options=['CONFIG_FILE={osmconf_path}']) AS osm
                WHERE osm.highway IS NOT NULL AND osm.name IS NOT NULL
        ...
        try:
            ...
            if not pqs:
                raise FileNotFoundError("⚠️ Nenhum arquivo Parquet de CNPJ encontrado na Bronze.")
