...
class TreinadorSafeDriver:
    ...
    def __init__(self):
        ...
        # Lê o segredo do GitHub; se vazio, assume 'safedriver'
        self.projeto = os.getenv("NOME_PROJETO", "safedriver").strip().lower()
        
        # 2. CONFIGURAÇÃO DE INFRAESTRUTURA (R2/S3)
        ...
        # 4. TELEMETRIA (Dossiê de Auditoria)
        self.telemetria = {
            "projeto": "SafeDriver",
    ...
    def _sincronizar_r2(self, local_path, s3_key):
        """Faz o upload dos artefatos para o Cloudflare R2."""
        print(f"☁️ Enviando {local_path} para o Data Lake: {s3_key}...", flush=True)
        self.s3.upload_file(local_path, self.bucket, s3_key)

    def carregar_dados(self):
        """Carrega a ABT Ouro garantindo que o caminho bata com o exportado pelo Arquiteto."""
        ...
        try:
            ...
            # Análise de densidade do sinal (Anti-Diluição)
            ...
            print(f"❌ ERRO CRÍTICO: Não foi possível ler {key_ouro}. Verifique os Secrets.")
            raise e

    def treinar(self):
        """Executa o ciclo de vida do treinamento CatBoost com Tweedie Loss."""
        ...
        # Amplifica sinais de crime para o modelo não 'viciar' no zero
        pesos = np.where(y > 0, 10.0, 1.0)

        # 3. Saneamento Categórico (Blindagem de Schema)
        for col in self.cat_features:
            if col in X.columns:
                X[col] = X[col].fillna("DESCONHECIDO").astype(str).str.replace(r'\.0$', '', regex=True)

        print(f"🚀 Iniciando CatBoost Tweedie (Instância: {self.projeto})...", flush=True)
        train_pool = Pool(X, y, cat_features=self.cat_features, weight=pesos)

        # 4. Hiperparâmetros de Alta Sensibilidade
        params = {
            'iterations': 3000,
            'learning_rate': 0.02,
            'depth': 8,              # Profundidade para captar micro-padrões geográficos
            'l2_leaf_reg': 1.5,      # Baixa regularização para permitir picos de risco
            'loss_function': 'Tweedie:variance_power=1.6', # Otimizado para cauda longa
            'eval_metric': 'MAE',
            'random_seed': 42,
            'verbose': 250,
            'task_type': 'CPU',
        ...
        # 5. Extração de Inteligência (Feature Importance)
        ...
        # 8. Sincronização com o Data Lake (R2)
        ...
        print(f"📑 Auditoria técnica salva em: modelos/{self.log_nome}")
