...
class IngestorSafeDriver:
    ...
    def _limpar_e_tipar(self, df: pl.DataFrame, ano: int, tempo_inicio_ano: float) -> pl.DataFrame:
        ...
        if "DATAREGISTRO" in df.columns:
            df = df.with_columns([
                pl.coalesce([
                    pl.col("DATAOCORRENCIA").str.slice(0, 10).str.strptime(pl.Date, format="%Y-%m-%d", strict=False),
                    pl.col("DATAOCORRENCIA").str.slice(0, 10).str.strptime(pl.Date, format="%d/%m/%Y", strict=False)
                ]).alias("_dt_oc"),
                pl.coalesce([
                    pl.col("DATAREGISTRO").str.slice(0, 10).str.strptime(pl.Date, format="%Y-%m-%d", strict=False),
                    pl.col("DATAREGISTRO").str.slice(0, 10).str.strptime(pl.Date, format="%d/%m/%Y", strict=False)
                ]).alias("_dt_re")
            ]).with_columns(
