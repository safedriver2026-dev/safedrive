...
class CamadaOuroSafeDriver:
    ...
    def executar_predicao_atual(self):
        try:
            ...
            # Gera as predições e prepara o DataFrame para o Esquema Estrela
            df_final = self._gerar_scores_ponderados(df_input, modelos)
            
            # Sincroniza Fato, Dimensões e View Analítica
            self._sincronizar_infraestrutura_bq(df_final)
            
            # Retorno seguro para o Webhook do Discord (evita o erro de list index)
            return {
                "embeds": [{
                    "title": "✅ SafeDriver - Sincronização Ouro Concluída",
                    "color": 3066993, 
                    "fields": [
                        {"name": "Registros Processados", "value": f"`{len(df_final):,}`", "inline": True},
                        {"name": "Status Camada Ouro", "value": "Esquema Estrela Atualizado", "inline": True},
                        {"name": "Motor de Predição", "value": "Ensemble Tweedie (CatBoost + LGBM)", "inline": False}
                    ]
                }]
            }
        except Exception as e:
    ...
    def _sincronizar_infraestrutura_bq(self, df):
        ...
        try:
            ...
            sql_init = f"CREATE TABLE `{tabela_fato}` AS SELECT H3_INDEX, SCORE_RISCO_GERAL, DT_ULTIMA_SINCRONIZACAO FROM `{tabela_staging}`"
        ...
        logger.info("OURO: Atualizando Dimensões e View Analítica...")
    ...
    def _build_star_schema(self, dataset, staging, fato):
        ...
        self.bq_client.query(f"""
            ...
            SELECT 
                f.H3_INDEX, f.SCORE_RISCO_GERAL, g.NM_BAIRRO, g.CIDADE, g.PERFIL_AREA, g.DENSIDADE,
        ...
        logger.info("OURO: Infraestrutura de dados pronta para consumo.")
