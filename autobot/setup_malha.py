...
def notificar_discord(webhook_url, mensagem):
    """Envia atualizações de status para o seu canal do Discord."""
    if webhook_url:
        try:
...
def processar_malha_referencia():
    ...
    try:
        ...
        df_h3 = pl.DataFrame({"id_h3_h9": list(hexs)}).with_columns([
            ...
            pl.col("id_h3_h9").map_elements(lambda x: h3.cell_to_latlng(x)[0]).alias("lat"),
            pl.col("id_h3_h9").map_elements(lambda x: h3.cell_to_latlng(x)[1]).alias("lon")
        ])
        ...
        print("🛣️ Vinculando nomes de ruas aos hexágonos correspondentes...")
        ...
        df_ruas_h3 = df_ruas_raw.with_columns(
            pl.struct(["r_lat", "r_lon"]).map_elements(lambda x: h3.latlng_to_cell(x["r_lat"], x["r_lon"], 9), return_dtype=pl.Utf8).alias("id_h3_h9")
        ...
        notificar_discord(WEBHOOK_SUCESSO, f"🏁 **Sucesso: Malha de Referência Gerada**\nTotal de Células: {len(df_referencia)}")
