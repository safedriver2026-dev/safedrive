...
def build_malha_mestra():
    ...
    try:
        ...
        df_raw = con.execute(f"""
            ...
            JOIN ST_Read('{paths['imediata']}') rgi ON ST_Within(ST_Point(h.lon, h.lat), rgi.geom)
            JOIN ST_Read('{paths['intermediaria']}') rint ON ST_Within(ST_Point(h.lon, h.lat), rint.geom)
            LEFT JOIN ST_Read('{paths['faces']}') f ON ST_DWithin(ST_Point(h.lon, h.lat), f.geom, 0.0001)
