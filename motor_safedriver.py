...
"""
Autobot SafeDriver — ETL & Previsões

Objetivo:
- Atualizar malha de risco para o app (Firestore) e base histórica para BI (Parquet)
- Evitar downloads desnecessários (cache + validação online)
- Não quebrar execução por falha pontual em ano específico (pula e segue)

Regras de robustez:
- Se não baixar e não houver parquet do ano -> pula ano e continua
- Se não baixar, mas houver parquet -> usa parquet
- Normaliza schema antes de salvar parquet (evita erro de tipo misto em NUM_BO)
"""
...
class SafeDriverEngine:
    """
    Autobot SafeDriver — Motor Principal
    - Ingestão SSP (RAW parquet por ano; ano atual validado online)
    - Dedup por NUM_BO usando refined do ano atual
    - Pesos e grid por geohash/perfil/período
    - XGBoost econômico (gated) + score_final
    - Firestore (app) + Parquet (BI) + métricas + resumo executivo
    """
    ...
    def __init__(self) -> None:
        ...
        self._session = self._http_session_com_retry()
    ...
    def executar_pipeline(self, ano_inicio: int = 2022, ano_fim: Optional[int] = None) -> None:
        ...
        if total_brutos == 0:
            campos = {
                ...
                "Status": "Nenhum dado foi carregado (sem RAW e sem download).",
                "Período": f"{ano_inicio}–{ano_fim}",
        ...
        # Janela recente 
        if TRAIN_WINDOW_MONTHS > 0 and "DATA_OCORRENCIA_BO" in df_pesos.columns:
    ...
    def _http_session_com_retry(self) -> requests.Session:
    ...
    def _parse_ssp_xlsx_to_df(self, xlsx_bytes: bytes, ano: int) -> pd.DataFrame:
        ...
        for aba in excel.sheet_names:
            ...
            # Normalização crítica para Parquet
            if "NUM_BO" in df_corrigido.columns:
                df_corrigido["NUM_BO"] = df_corrigido["NUM_BO"].astype("string")

            if "DATA_OCORRENCIA_BO" in df_corrigido.columns:
                df_corrigido["DATA_OCORRENCIA_BO"] = df_corrigido["DATA_OCORRENCIA_BO"].astype("string")

            if "HORA_OCORRENCIA_BO" in df_corrigido.columns:
                df_corrigido["HORA_OCORRENCIA_BO"] = df_corrigido["HORA_OCORRENCIA_BO"].astype("string")

            for c in ["NATUREZA_APURADA", "NOME_MUNICIPIO", "DESCR_TIPOLOCAL", "DESCR_SUBTIPOLOCAL"]:
                if c in df_corrigido.columns:
                    df_corrigido[c] = df_corrigido[c].astype("string")

            for c in ["LATITUDE", "LONGITUDE"]:
                if c in df_corrigido.columns:
                    df_corrigido[c] = pd.to_numeric(df_corrigido[c], errors="coerce").astype("float64")

            df_corrigido["ANO_BASE"] = int(ano)
            df_bruto = pd.concat([df_bruto, df_corrigido], ignore_index=True)

        return df_bruto

  
    def _carregar_dados_ssp(self, ano: int) -> Optional[pd.DataFrame]:
        """
        Retorna:
        - DataFrame quando conseguiu (download ou cache)
        - None quando não conseguiu baixar e não existe parquet (pula ano)
        """
        ...
        if ano != ano_atual and out_path.exists() and not SSP_FORCE_REFRESH:
            ...
            logging.info("Autobot SafeDriver: SSP %s usando cache Parquet.", ano)
            return pd.read_parquet(out_path)

   
        if out_path.exists() and not SSP_FORCE_REFRESH:
            try:
                h = self._session.head(url, timeout=30, allow_redirects=True)
                if h.ok:
        ...
        try:
            ...
            resp = self._session.get(url, timeout=120, headers=headers, allow_redirects=True)
            ...
            if out_path.exists():
                logging.warning(
                    "Autobot SafeDriver: falha ao baixar SSP %s (%s). Usando cache Parquet existente.",
                    ano, e
                )
                self.exec_meta["ssp_status"] = f"SSP {ano}: falha online, fallback para cache Parquet"
                return pd.read_parquet(out_path)

            
            logging.warning(
                "Autobot SafeDriver: SSP %s indisponível (sem download e sem cache). Pulando este ano. Erro=%s",
                ano, e
            )
            return None
    ...
    def _geohash_bbox_center(self, geohash_s: str) -> Tuple[float, float, float, float, float, float]:
        lat, lon, lat_err, lon_err = gh.decode_exactly(geohash_s)
        min_lat = lat - lat_err
        max_lat = lat + lat_err
        min_lon = lon - lon_err
        max_lon = lon + lon_err
    ...
    def _format_modalidades(self, modalidades: Dict[str, Dict[str, Any]]) -> str:
        ...
        for perfil, m in items:
            linhas.append(f"• {perfil}: {m.get('linhas',0):,} ({m.get('pct',0):.1f}%) | BOs: {m.get('bos_unicos',0):,}")
        return "\n".join(linhas)
    ...
    def _gerar_resumo_executivo_motor(
        ...
        return {
            "🤖 Autobot SafeDriver": "Resumo Executivo —ETL & Previsões",
            "📡 SSP": self.exec_meta.get("ssp_status", "Status SSP indisponível"),
            "🆕 BOs novos (ano atual)": f"{novos_bo:,}",
            "📦 Brutos": f"{total_brutos:,}",
            "✅ Qualificados": f"{total_qualificados:,} ({aproveitamento:.1f}%)",
            "🧩 Modalidades": self._format_modalidades(modalidades),
            "🕒 Modalidade x Período (Top)": self._format_modalidades_periodo_top(modalidades_periodo, top_n=6),
            "🧠 XGBoost": self.exec_meta.get("xgb_status", "Status XGB indisponível"),
            "🧱 Grid": f"{grid_size:,} células",
            "📊 Parquet refined": f"+{events_parts_written} part(s)",
            "🗺️ Firestore": "Atualizado para o app" if self.db else "Desativado",
            "⏱️ Tempo": f"{tempo_s}s",
            "🗓️ Conclusão": referencia.strftime("%d/%m/%Y %H:%M"),
        }
    ...
    def _enviar_resumo_discord_motor(self, campos: Dict[str, Any], status: str = "sucesso") -> None:
        ...
        titulo = {
            "sucesso": "Autobot SafeDriver — Motor | Execução concluída",
            "neutro": "Autobot SafeDriver — Motor | Execução informativa",
            "erro": "Autobot SafeDriver — Motor | Falha detectada",
        }.get(status, "Autobot SafeDriver — Motor")

        fields = [{"name": str(k), "value": str(v)[:1024], "inline": False} for k, v in campos.items()]
        payload = {
            ...
            "embeds": [{
                ...
                "fields": fields,
                "footer": {"text": "Autobot SafeDriver • Cache Parquet • Firestore app grid"},
            }]
        }
