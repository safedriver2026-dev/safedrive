...
def test_check_ouro():
    ...
    if not p.exists(): pytest.skip("Arquivo ausente")
    df = pl.read_parquet(p)
    assert all(c in df.columns for c in ["ESCORE_RISCO", "GEOMETRIA_WKT", "CODIGO_H3"])

def test_filtro_sp():
    p = Path("datalake/ouro/dashboard_final.parquet")
    if not p.exists(): pytest.skip("Arquivo ausente")
    df = pl.read_parquet(p)
    f = df.filter((pl.col("LATITUDE_MEDIA") < -25.5) | (pl.col("LATITUDE_MEDIA") > -19.5))
    assert f.height == 0
